...
const GITHUB_MUTATING_TOOLS = new Set([
  "add_issue_comment",
  "create_branch",
  "create_issue",
...
const CATALOG_POLICY_VERSION = "2026-07-19.1";
...
export function catalogConnectorOrigin(catalogId: CatalogConnectorId): ConnectorOrigin {
  return { kind: "catalog", id: catalogId, version: CATALOG_POLICY_VERSION };
}

export function migrateLegacyCatalogConnector(
  connector: ConnectorConfig,
): ConnectorConfig | undefined {
  const catalogId = CATALOG_IDS.find(
...
export function catalogConnectorRuntime(
  connector: ConnectorConfig,
...
export function connectorToolRisk(connector: ConnectorConfig, tool: string): ConnectorRisk {
  ...
  if (connector.origin.id === "github") {
    ...
    return GITHUB_MUTATING_TOOLS.has(tool) ? "mutating" : "critical";
  }
