...
import { ChevronDown, Clock, Pause, Play, Plus } from "@/ui/icon-registry";
    ...
    <div className="flex shrink-0 flex-col pt-3">
      ...
      >
        <button
          ...
          className="ml-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-md text-(--dim) transition-[background-color,color,transform] hover:bg-(--hover) hover:text-(--fg) active:scale-[0.96]"
          aria-label={expanded ? "Collapse automations" : "Expand automations"}
          aria-expanded={expanded}
          aria-controls="agent-sidebar-automations"
          title={expanded ? "Collapse automations" : "Expand automations"}
        >
          <ChevronDown
        ...
        <Link
          ...
          prefetch={false}
          className="flex h-full min-w-0 flex-1 items-center gap-2 text-[length:var(--fs-sm)] font-normal text-(--hl2) hover:text-(--fg)"
        >
      ...
      <div id="agent-sidebar-automations" hidden={!expanded}>
        {automations.map((automation) => (
          ...
          >
            ...
            <div className="absolute right-1.5 top-1/2 flex -translate-y-1/2 items-center gap-0.5 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100">
              ...
              >
                {pendingId === automation.id ? <Spinner size="xs" /> : <Play className="h-3 w-3" />}
              </button>
              <button
                type="button"
                disabled={pendingId === automation.id}
