...
const buttonVariant = {
  primary: "rounded-lg bg-(--accent)/15 text-(--accent) hover:bg-(--accent)/25",
  secondary:
    "rounded-lg border border-(--border)/40 text-(--dim) hover:bg-(--fg)/[0.06] hover:text-(--fg)",
  ghost: "rounded-lg text-(--dim) hover:bg-(--fg)/[0.06] hover:text-(--fg)",
  danger: "rounded-lg text-(--err) hover:bg-(--err)/15",
